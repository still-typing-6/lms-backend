...
export const updateCourse = async (updateDetail: updateCourseDetail, courseId: number) => {
  const result = await db.update(courseTable).set({ courseName: updateDetail.courseName, description: updateDetail.description }).where(eq(courseTable.courseId, courseId));
  return result;
}
...
export const deactiveCourse = async (courseId: number) => {
  const result = await db.update(courseTable).set({ isActive: false }).where(eq(courseTable.courseId, courseId));
  return result;
}
